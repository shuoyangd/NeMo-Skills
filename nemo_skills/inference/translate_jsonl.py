...
from nemo_skills.utils import setup_logging, nested_dataclass
...
class TranslationTask(GenerationTask):
    ...
    async def process_single_datapoint(self, data_point, all_data):
        """Override to handle inference config that might be a dict instead of dataclass."""
        from nemo_skills.inference.generate import combine_stop_phrases
        ...
        generate_params = {
            ...
            "stop_phrases": combine_stop_phrases(
                self.prompt.stop_phrases if self.prompt is not None else None, getattr(self, 'extra_stop_phrases', [])
            ),
            **inference_params,
            **getattr(self, 'extra_generate_params', {}),
        }

        if self.cfg.code_execution:
            if (
                hasattr(self.cfg, 'override_max_code_executions')
                and self.cfg.override_max_code_executions
                and hasattr(self.cfg, 'total_code_executions_in_prompt')
                and self.cfg.total_code_executions_in_prompt is not None
            ):
                max_code_executions_values = [data_point.get('total_code_executions', 1)]
                generate_params["max_code_executions"] = max_code_executions_values

        return await self.llm.generate_async(**generate_params)
